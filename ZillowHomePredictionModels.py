...
XGB1_WEIGHT = 0.90738#0.8083  # Weight of first in combination of two XGB models
...
class ZillowHomePredictionModels:
    ...
    def generate_combined_xgb_pred(self, x_train, y_train, x_test):
        ...
        xgb_train_pred1 = self.__get_model_prediction__(xgb_model1, xgb.DMatrix(x_train))
        xgb_test_pred1 = self.__get_model_prediction__(xgb_model1, dtest)
        #xgb1_oof_train, xgb1_oof_test = self.get_oof(xgb_model1, x_train, y_train, x_test)

        xgb_model2 = self.generate_xgb_model(x_train, y_train, xgb_params2, 150)
        xgb_train_pred2 = self.__get_model_prediction__(xgb_model2, xgb.DMatrix(x_train))
        xgb_test_pred2 = self.__get_model_prediction__(xgb_model2, dtest)
        #xgb2_oof_train, xgb2_oof_test = self.get_oof(xgb_model1, x_train, y_train, x_test)

        new_x_train = pd.DataFrame({"model1": xgb_train_pred1, "model2": xgb_train_pred2})
        new_x_test = pd.DataFrame({"model1": xgb_test_pred1, "model2": xgb_test_pred2})
        #combined_xgb_model = self.generate_xgb_model(new_x_train, y_train, xgb_params1, 150)
        combined_xgb_pred = XGB1_WEIGHT*xgb_test_pred1 + (1-XGB1_WEIGHT)*xgb_test_pred2
